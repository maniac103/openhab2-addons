...
public class Clip2Bridge implements Closeable {
    ...
    private static class DataFrameCollector {
        ...
        public void append(ByteBuffer data) {
            int neededSize = usedSize + data.capacity();
            if (neededSize > buffer.length) {
                int newSize = Math.max(2 * buffer.length, neededSize);
                buffer = Arrays.copyOf(buffer, newSize);
            }
            data.get(buffer, usedSize, data.capacity());
            usedSize += data.capacity();
        }
    ...
    private class EventStreamListenerAdapter extends BaseStreamListenerAdapter<Boolean> {
        ...
        public void onData(@Nullable Stream stream, @Nullable DataFrame frame, @Nullable Callback callback) {
            ...
            synchronized (this) {
                ...
                // a blank line marks the end of an SSE packet
                boolean endOfMessage = !receivedLines.isEmpty() && receivedLines.get(receivedLines.size() - 1).isBlank();

                if (endOfMessage) {
                    eventData.reset();
                    // collect event content from all 'data' fields in this message
                    ...
                    for (String receivedLine : receivedLines) {
                        ...
                        } else if (receivedLine.equals(": hi") && !completable.isDone()) {
